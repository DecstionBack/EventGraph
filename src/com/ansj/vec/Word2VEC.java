...
public class Word2VEC {
	...
	public Set<WordEntry> distance(String queryWord) {
		...
		//看具体的词向量什么意思？？
		System.out.println(queryWord + " center:");
		...
		for (Map.Entry<String, float[]> entry : wordMap.entrySet()) {
			//自己加的
			if(entry.getKey().equals("倾覆")){
				System.out.print("倾覆:");
				for (int k = 0; k < entry.getValue().length; k++)
					System.out.print(entry.getValue()[k] + " ");
			System.out.println();}
	...
	public double fileDistance(List<String> list1, List<String> list2){
		float[] vector1 = new float[size], vector2 = new float[size];
		for (String str : list1){
			for (int i = 0; i < size; i++)
				vector1[i] += wordMap.get(str)[i];
		}
		for (String str : list2){
			for (int i = 0; i < size; i++)
				vector2[i] += wordMap.get(str)[i];
		}
		double multi = 0, square1 = 0, square2 = 0;
		for (int i = 0; i < size; i++){
			multi += vector1[i] * vector2[i];
			square1 += vector1[i]*vector1[i];
			square2 += vector2[i]*vector2[i];
		}
		return multi / (Math.sqrt(square1) * Math.sqrt(square2));  
	}
